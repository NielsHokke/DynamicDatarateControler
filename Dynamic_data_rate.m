...
if ForceLearning == 1
    numPackets = 10000; % Number of packets transmitted during the training
else
...
if walkSNR
    ...
    baseSNR = sin(linspace(1,10,numPackets)*round(numPackets/100))*amplitude+meanSNR;
...
for numPkt = 1:numPackets 
    waitbar(numPkt/numPackets,f,sprintf('Simulating packet  %u/%u',numPkt, numPackets));
    if walkSNR
    ...
    %Code to caclulate instantanious datarate 
    if ber(numPkt)==0
        speed(numPkt) = 8*cfgVHT.APEPLength/(y.RxWaveformLength/sr)/1e6;
    else
        speed(numPkt) = 0;
    end
    
    if round(LearnedValues(MCS_custom+1,floor(y.EstimatedSNR/SNRBinSize)+1)) ~= round(speed(numPkt))
        % disp("Difference")
        % disp(speed(numPkt))
        % disp(LearnedValues(MCS_custom+1,floor(y.EstimatedSNR/SNRBinSize)+1))
        if LearnedValues(MCS_custom+1,floor(y.EstimatedSNR/SNRBinSize)+1) == -1;
            LearnedValues(MCS_custom+1,floor(y.EstimatedSNR/SNRBinSize)+1) = speed(numPkt);
        else
                LearnedValues(MCS_custom+1,floor(y.EstimatedSNR/SNRBinSize)+1) = speed(numPkt)*AdaptionRate + (1-AdaptionRate)*LearnedValues(MCS_custom+1,floor(y.EstimatedSNR/SNRBinSize)+1);
        end
       LearnedValues(MCS_custom+1,floor(y.EstimatedSNR/SNRBinSize)+1);
    else
        % disp("Same value found")
    end

    
    
    [M, I] = max(LearnedValues);
    MCS_custom = I(floor(y.EstimatedSNR/SNRBinSize))-1;
 
    if rand() < LearningSpeed
        disp("randomizing by")
        MCS_custom = MCS_custom + 1
    end 
    ...
    if ForceLearning == 1
        MCS_custom = floor(mod(numPkt,1000)/100);
    end
    ...
    MCS(numPkt) = MCS_custom; % cfgVHT.MCS; % Store current MCS value
    cfgVHT.MCS =  MCS_custom; %  snrInd-1;

    % disp( 8*cfgVHT.APEPLength/(y.RxWaveformLength/sr)/1e6); % y.RxWaveformLength/sr = packetlength
    % henk = cfgVHT;
end
close(f)
